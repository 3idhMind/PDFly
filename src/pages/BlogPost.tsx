...
PDFly is a free PDF generation platform with a REST API, plus a browser tool that does everything client-side — your files never leave your machine. 15 templates, 11 page sizes, batch generation, and image-to-PDF conversion with 25+ formats. The browser tool also renders Hindi, Arabic and Chinese, loading the right Noto font automatically; the API renders Latin scripts today. Developer-friendly examples in JavaScript, Python, PHP, Go, and cURL. Free with no credit card required.

Best for: API-first PDF generation, private in-browser conversion, image-to-PDF conversion.
...
For most developers in 2026, we recommend starting with PDFly for API-based generation (especially if you need image-to-PDF, or want a browser tool that never uploads the file), jsPDF for simple client-side needs, and Puppeteer when you need pixel-perfect web page rendering. Each tool has its strengths — pick the right one for your use case.`,
...
5. Keep API content in Latin scripts for now — the API renders with the built-in Helvetica. For Devanagari, Arabic or Chinese, use the browser tool at /app
...
Privacy: The browser tool is 100% client-side — files are never uploaded
Scripts: Hindi, Arabic and Chinese in the browser tool, with the right font fetched automatically. The API renders Latin scripts today
...
Self-hosted: No (cloud API), but the source is open and auditable
Best for: Developers, SaaS integration, automation, private in-browser conversion
...
Files never uploaded (web tool):
- PDFly: ✓ (100% client-side)
- Adobe: ✗ (online tools upload)
- SmallPDF: ✗ (online tools upload)
...
For developers: PDFly wins hands-down. Free API, excellent documentation, batch processing, and an open, auditable codebase make it the best choice for integration into applications.
...
For privacy-sensitive work: PDFly is the only one of the three whose web tool never uploads your file. Everything happens in the browser tab, which also means Hindi, Arabic and Chinese render there without you installing a font — the right Noto font is fetched on demand. Our REST API renders Latin scripts today; we'd rather say so here than have you find out in production.
...
There are 15 professional templates to choose from, 11 page sizes, and the API handles batch generation of up to 10 documents per request. One thing to know up front: the API renders with the built-in Helvetica, so it covers Latin scripts today. If your content is Devanagari, Arabic or Chinese, use the browser tool at /app — it fetches a matching Noto font before rendering.
...
What PDFly covers, script by script

The browser tool picks a font from the script your chosen language uses, not from the language itself — one Noto font covers a whole script:

Devanagari — Noto Sans Devanagari, used for Hindi, Marathi, Sanskrit and Nepali
Arabic script — Noto Sans Arabic, used for Arabic, Persian and Urdu
Simplified Chinese — Noto Sans SC
Latin — the built-in fonts, which already handle English, Spanish, French, German, Portuguese, Italian and the rest

The font is fetched from a CDN and embedded into the PDF only when your text actually needs it, so a plain English document stays small.

Where this works today — and where it doesn't

Being precise about this matters more than sounding impressive, so:

The browser tool at /app handles non-Latin scripts properly. Pick a language, and it detects the script in your content and fetches a matching Noto font before rendering — Devanagari for Hindi, Marathi, Sanskrit and Nepali; the Arabic script for Arabic, Persian and Urdu; Simplified Chinese. Nothing to install, and the font is only downloaded if your text actually needs it.

The REST API does not do this yet. It renders with the built-in Helvetica, which covers Latin scripts only, so Devanagari or Arabic sent to the API will not come out correctly. If you need those scripts today, use the browser tool. We would rather tell you that here than let you find it in production.

Making the API match the browser means shipping font subsets server-side and adding proper bidirectional text handling for right-to-left scripts. It is on the roadmap and it is real work, not a switch we forgot to flip.

Japanese, Korean and Thai are not covered in either path yet.`,
...
All 15 templates work with all 11 page sizes, and none of them stamp a watermark on your document. You can switch templates between documents in a batch request — so one API call can produce both a professional invoice and a creative portfolio piece.`,
...
No watermark, ever — the PDF you get is the PDF you shipped
...
Start building with PDFly today — create your free account and generate your first PDF in under a minute.`,
  ...
  "invoice-pdf-generation-tutorial": `Automating invoice generation is one of the most requested features for SaaS applications, e-commerce platforms, and freelance tools. With PDFly, you can generate professional invoices as PDFs with any of the 15 templates, through a simple API call.
...
A note on symbols and scripts: the API renders with the built-in Helvetica, which covers $, €, £ and ¥ but not ₹ or any non-Latin script. For a Hindi invoice, or one priced in rupees using the ₹ sign, generate it in the browser tool at /app — it loads a matching font first. Writing "INR 1,200" keeps you safe either way.
...
Pros: Zero infrastructure — just API calls, 15 templates included, 11 page sizes, batch processing, rate limiting and API key management, analytics dashboard, no watermark, completely free and open source
...
- PDFly API: Latin scripts today (Hindi, Arabic and Chinese work in the browser tool, which loads the font for you)

Our recommendation: For new projects, start with PDFly. It's completely free, requires zero setup, and handles batch processing out of the box. If your documents are in Devanagari, Arabic or Chinese, use the browser tool rather than the API for now. If you need to migrate later, the REST API format makes it easy to swap backends.`,
...
- If your users write in non-Latin scripts, send them to the browser tool — the API renders Latin scripts today
...
Where PDFly stands today

Being straight about this matters more than sounding impressive, so here is exactly how far we get.

The browser tool at /app solves the font problem. Choose Arabic, Persian or Urdu and it detects the Arabic script in your content, fetches Noto Sans Arabic, and embeds it into the PDF. You get real Arabic glyphs instead of the empty boxes most generators hand you, and there is nothing to install.

It does not solve the other three problems on this page. There is no Unicode bidirectional algorithm and no Arabic glyph shaping anywhere in the codebase. Characters are drawn in the order your content supplies them, in their isolated forms, so letters do not join. For a short heading or label whose order you control, that is workable. For a paragraph of running Arabic prose, it is not — and you should know that before you ship, not after.

Hebrew is a separate script with a separate font, and we do not load it yet.

The REST API does none of this. It renders with the built-in Helvetica, which covers Latin scripts only, so RTL content sent to the API comes back garbled or dropped. Use the browser tool.

Getting the rest right means a shaping engine to select positional forms and ligatures, a bidi implementation to resolve mixed-direction runs, and a font per script — server-side as well as in the browser. It is real work rather than a switch we forgot to flip, and it is on the roadmap. Until it lands, this page will keep saying so.`,
...
If your certificates are in Hindi or another Devanagari language, generate them in the browser tool at /app, which loads the matching Noto font for you. The batch API renders with the built-in Helvetica and covers Latin scripts today.
