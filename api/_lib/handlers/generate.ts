...
import { prepareFont } from "../fonts.js";
...
async function renderPdf(
  ...
  /*
   * Register a Noto font when the content is not Latin. Must happen before any
   * text is drawn: jsPDF resolves the family at draw time, so a font added
   * later applies to nothing already on the page.
   *
   * Never throws. A CDN outage downgrades to Latin with a warning rather than
   * failing a request that would otherwise have produced a good English PDF.
   */
  const font = await prepareFont(pdf, `${doc.title}\n${doc.content}`, language);
  ...
  return { bytes: new Uint8Array(pdf.output("arraybuffer")), warnings: font.warnings };
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    /*
     * Re-check the quota now that the batch size is known.
     *
     * The check above runs before the body is parsed, so it can only ask "has
     * this account got anything left". That let a caller at 99 of 100 send five
     * documents and finish at 104. Cheap to ask twice: the first call rejects
     * an exhausted account without parsing anything, this one refuses a batch
     * that would not fit.
     */
    const batchQuota = await checkQuota(caller.uid, documents.length);
    if (!batchQuota.allowed) {
      return fail(
        res,
        429,
        "LIMIT_EXCEEDED",
        `This request needs ${documents.length} document${documents.length === 1 ? "" : "s"} and ` +
          `only ${batchQuota.remaining} remain in your monthly allowance of ${batchQuota.limit}.`,
        { used: batchQuota.used, limit: batchQuota.limit, remaining: batchQuota.remaining },
      );
    }
    ...
    for (let di = 0; di < documents.length; di++) {
      ...
      const rendered = await renderPdf(doc, finalTemplate, pageDims, isRawMode, language as string);
      const pdfBytes = rendered.bytes;
      // Font problems are reported, never swallowed: a document that quietly
      // dropped its Devanagari is worse than one that says it did.
